...
def main():
    ...
    while ans :
        ...
        if ans=="1":
            os.system('cls')
            print("\tINICIO DE ENCRIPTADO")
            inputfile = input("\n Escriba el nombre del archivo a encriptar: \n")
            outputfile = input("Escriba el nombre del archivo tras ser encriptado:\n")
            kfile = input("Escriba el nombre del archivo que contiene la llave:\n")
            with open(kfile, 'rb+') as key:
                k = key.read()
            w = int(input("Digite el Block-size {32, 64,128 bits}:\n"))
            r = int(input("Digite la cantidad de rondas que desea que el algoritmo encripte {0-255}:\n"))

            a = RC5(w,r,k)
            a.encryptFile(inputfile ,outputfile)
            os.system('pause')
            continue

        elif ans=='2':
            os.system('cls')
            print("\tINICIO DE DESENCRIPTADO")
            dinputfile = input("\n Escriba el nombre del archivo a desencriptar: \n")
            doutputfile = input("Escriba el nombre del archivo tras ser desencriptado:\n")

            kfile = input("Escriba el nombre del archivo que contiene la llave:\n")
            with open(kfile, 'rb+') as key:
                k= key.read()

            w = int(input("Digite el Block-size {32, 64,128 bits} que fue utilizado:\n"))
            r = int(input("Digite la cantidad de rondas que se utilizaron para encriptar {0-255}:\n"))

            a = RC5(w,r,k, strip_extra_nulls=True)
            a.decryptFile(dinputfile, doutputfile)
            key.close()
            os.system('pause')
            continue
